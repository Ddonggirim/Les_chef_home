...
public interface AllCommentRepository extends JpaRepository<AllComment, Long> {
    ...
    @Query(value = "select * from All_Comment where nickname = :commenter and article_id is null", nativeQuery = true)
    List<AllComment> findMyRecipeComment(@Param("commenter") String commenter);
    @Query(value = "select * from All_Comment where nickname = :commenter and recipe_id is null", nativeQuery = true)
